...
bool RingParser::DecodeSlot(QueueState* qs, uint64_t id, PacketRecord* rec) {
  ...
  if (!ReadProcMem(q.pid, addr, raw, sizeof(raw))) {
    if (kDebug) fprintf(stderr, "[decode q%lu id%lu] read failed @0x%lx\n",
                        q.uid, id, addr);
    return false;
  }
  ...
  // Producer publishes the header (with release) last. Treat not-yet-published
  // slots as pending so we don't decode a half-written packet.
  if (type == aql::PacketType::Invalid || type == aql::PacketType::VendorSpecific) {
    if (kDebug) fprintf(stderr, "[decode q%lu id%lu] not-ready header=0x%x\n",
                        q.uid, id, header);
    return false;
  }
...
void RingParser::PollQueue(QueueState* qs, double now) {
  ...
  if (!ReadU64(q.pid, q.wptr_addr, &wptr)) return;
  if (!ReadU64(q.pid, q.rptr_addr, &rptr)) return;
  ...
  if (kDebug && (wptr != qs->next_id || rptr != qs->last_rptr)) {
    fprintf(stderr, "[poll q%lu] wptr=%lu rptr=%lu next_id=%lu inflight=%zu\n",
  ...
  // Decode newly enqueued packets in order. The producer bumps write_dispatch_id
  // when it *reserves* a slot, then publishes the header (type) last, so a slot
  // with id < wptr may momentarily read as INVALID. Distinguish two cases:
  //   * id >= rptr : still pending publish -> stop and retry next poll.
  //   * id <  rptr : already consumed and we missed the publish window
  //                  (kernel finished between polls) -> count as dropped, skip.
  ...
  // Complete anything the GPU has consumed (read_dispatch_id advanced past it).
  while (!qs->inflight.empty()) {
    auto it = qs->inflight.begin();
    if (it->first >= rptr) break;
...
void RingParser::Run() {
  while (running_) {
    ...
    {
      std::lock_guard<std::mutex> lk(mu_);
      for (auto& q : queues_) snapshot.push_back(q.get());
    }
    for (QueueState* qs : snapshot) {
      if (!ProcAlive(qs->info.pid)) continue;
      PollQueue(qs, now);
    }
    usleep(poll_us_);
  }
