...
std::string PeekStr(const MemReader& m, uint64_t va, size_t max = 512) {
  ...
  char buf[64];
  while (s.size() < max) {
    if (!m.Read(va + s.size(), buf, sizeof(buf))) break;
...
std::vector<uint64_t> FindElfBases(int pid, const MemReader& m) {
  ...
  while (std::getline(f, line)) {
    ...
    char perms[8] = {0};
    if (sscanf(line.c_str(), "%lx-%lx %7s %lx", &start, &end, perms, &off) < 4)
      continue;
    if (perms[0] != 'r') continue;
    // NB: AMDGPU code objects backed by /dev/dri/renderD128 report a nonzero
    // file offset (renderD128 is one large BO), yet each object's ELF header
    // still sits at the mapping start. So we probe every readable region start
    // for ELF magic rather than filtering on offset 0.
    unsigned char ident[20];
    if (!m.Read(start, ident, sizeof(ident))) continue;
    if (memcmp(ident, ELFMAG, SELFMAG) != 0) continue;
    uint16_t machine = *reinterpret_cast<uint16_t*>(ident + 18);
    if (machine == EM_AMDGPU) bases.push_back(start);
  }
  return bases;
}
...
static void ScanImage(const MemReader& pid, uint64_t base,
  ...
  for (int i = 0; i < eh.e_phnum; ++i) {
    Elf64_Phdr ph;
    if (!Peek(pid, base + eh.e_phoff + i * sizeof(ph), &ph)) return;
  ...
  for (uint64_t i = 0; i < n; ++i) {
    Elf64_Dyn d;
    if (!Peek(pid, bias + dyn_vaddr + i * sizeof(d), &d)) break;
  ...
  if (!symtab || !strtab || !syment) return;

  // Dynamic pointers may be link-time vaddrs; rebase if below the load address.
...
void KernelSymbolResolver::ScanCodeObjects() {
  ...
  for (uint64_t base : bases) ScanImage(mem, base, &cache_);
  ++scanned_generation_;
  if (getenv("HSA_SNOOP_DEBUG"))
    fprintf(stderr, "[ksym pid=%d] scanned %zu AMDGPU images, %zu .kd symbols "
            "(+%zu)\n", pid_, bases.size(), cache_.size(),
            cache_.size() - before);
}
...
std::string KernelSymbolResolver::Resolve(uint64_t kernel_object) {
  ...
  if (getenv("HSA_SNOOP_DEBUG")) {
    fprintf(stderr, "[ksym pid=%d] MISS kernel_object=0x%lx; sample cache:\n",
            pid_, kernel_object);
    int n = 0;
    for (auto& kv : cache_) {
