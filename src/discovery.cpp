...
Discovery::Discovery(std::string tracefs_root, int pid_filter)
    : tracefs_(std::move(tracefs_root)),
...
bool Discovery::InstallProbe() {
  ...
  //   queue_type           +40  u32
  std::string def =
      "p:" + probe_name_ + " kfd_ioctl_create_queue"
      " ring_base=+0(%dx):u64"
      " wptr=+8(%dx):u64"
      " rptr=+16(%dx):u64"
      " ring_size=+32(%dx):u32"
  ...
  if (!WriteFile(kp, def, /*append=*/false)) {
    fprintf(stderr, "hsa-snoop: failed to install kprobe (%s). Need root?\n",
            strerror(errno));
    return false;
  }
  if (!WriteFile(tracefs_ + "/events/kprobes/" + probe_name_ + "/enable", "1\n",
                 false)) {
...
void Discovery::Run() {
  ...
  while (running_) {
    ...
    while ((nl = buf.find('\n')) != std::string::npos) {
      ...
      if (line.find(probe_name_) == std::string::npos) continue;

      // Parse "<comm>-<pid> [cpu] ..." prefix.
      size_t cpu = line.find(" [");
      if (cpu == std::string::npos) continue;
      ...
      if (s != std::string::npos) comm = comm.substr(s);

      // The ftrace comm can be a transient value (e.g. "<...>") if the queue is
      // created mid-exec. Prefer the authoritative /proc/<pid>/comm.
      char cpath[64];
      ...
      if (pid_filter_ && pid != pid_filter_) continue;
      ...
      if (q.ring_base && q.ring_size && cb_) cb_(q);
    }
  }
}
