...
// Reads /opt/rocm/.info/version or parses rocm_version.h; returns "unknown" on failure.
...
// On WSL2 (no KFD sysfs): parses `rocminfo` for the GFX target string (e.g. "gfx1151").
...
class PrometheusExporter {
  ...
  private:
    ...
    std::unordered_map<std::string, prometheus::Gauge*> active_queue_gauges_;   // gpu_id_str -> gauge
    std::unordered_map<std::string, prometheus::Gauge*> last_launch_ts_gauges_; // gpu_id_str -> gauge
    std::unordered_map<std::string, LastKernelState>    last_kernel_state_;     // gpu_id_str -> state
    ...
    std::unordered_map<std::string, prometheus::Gauge*> rate_gauges_; // gpu_id_str -> gauge
